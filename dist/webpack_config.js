...
function _entry(dirs = []) {
    return dirs.reduce((memo, dir) => {
        const packs = new glob_1.GlobSync(path_1.resolve(dir, '**/*.pack.+(s[ac]ss|js)'));
        packs.found.forEach((pack) => {
            const key = path_1.relative(dir, pack).replace(/\.pack\.[^/.]+$/, '');
...
function config(mode = 'production', assets = [], modules = [], outputDir = false) {
    ...
    const context = path_1.resolve(__dirname, '..', 'node_modules');
            ...
            rules: [
                {
                    ...
                    use: [
                        ...
                        { loader: 'sass-loader', options: { implementation: sass_1.default, sourceMap: true } },
...
//# sourceMappingURL=webpack_config.js.map