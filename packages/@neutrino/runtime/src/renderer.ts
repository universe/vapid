...
import type { InsertPosition, NodeType, SimpleDocument, SimpleDocumentFragment, SimpleElement, SimpleNode,SimpleText  } from '@simple-dom/interface';
import { uuid } from '@universe/util';

import { HelperResolver } from './helpers';
import { GlimmerTemplate, IParsedTemplate, RendererComponentResolver } from './types.js';
  ...
  switch(node.path.type) {
    case 'PathExpression':
      ...
      if (node.path.parts.length === 1 && !node.path.this && !node.path.data) {
        ...
        if (helper.prototype instanceof CollectionHelper && !params[1]) {
 ...
 */
export function render(document: SimpleDocument, tmpl: IParsedTemplate, resolveComponent: RendererComponentResolver, resolveHelper: HelperResolver, context = {}, data = {}) {
  const ast = tmpl.ast;
