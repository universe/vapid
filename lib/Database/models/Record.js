    ...
    return super.init({
      ...
      getterMethods: {
        ...
        name() {
          ...
          if (template.type === 'page') {
            return this.get('metadata').name || this.defaultName;
          }
          return this.get('metadata').name || this.get('slug') || this.defaultName;
        },
        ...
        safeSlug() {
          const { template } = this;
          const customSlug = (this.getDataValue('slug') || '').replace(`{${template.id}}`, '');
      ...
      hooks: {
        ...
        async beforeSave(record) {
          ...
          // Prefix collection slugs with the collection ID to namespace.
          ...
          if (record.metadata.slug && template.type === 'collection') {
            record.metadata.slug = `{${template.id}}${record.metadata.slug}`;
          }
