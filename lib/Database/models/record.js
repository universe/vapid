...
class Record extends Sequelize.Model {
  ...
  async permalink() {
    ...
    let slug = this.getDataValue('permalink') ? Utils.kebabCase(this.getDataValue('permalink')) : '';

    if (template.type === 'collection') {
      const name = Utils.kebabCase(this.dataValues.title || this.dataValues.name);
      slug = name ? `${name}-${this.id}` : this.id;
      return `/${template.name}/${slug}`;
    } else if (template.type === 'page') {
      const templateName = template.name === 'index' ? '' : template.name;
      slug = slug || templateName;
      return `/${slug}`;
    }
  ...
  async title() {
    ...
    return Utils.startCase(this.dataValues.metadata.title || templateName);
  }
