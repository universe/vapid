import { Template, ITemplate, PageType, Record as DBRecord, IRecord, Record  } from './models';
...
export default class Database<T extends { type: string; } = { type: string; }> extends IProvider<T> {
  ...
  saveFile(name: string, file: NodeJS.ReadableStream): Promise<string> { return this.provider.saveFile(name, file); }
