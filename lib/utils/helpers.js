...
const Utils = {
  ...
  startsWith: (str, test) =>  str.indexOf(test) === 0,
  endsWith: (str, test) => str.slice(str.length - test.length) === test,
  ...
  camelCase: str => {
    let s = str && str.match(STR_SPLIT_REG).map(x => x.slice(0, 1).toUpperCase() + x.slice(1).toLowerCase()).join('');
