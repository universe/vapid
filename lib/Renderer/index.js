...
exports.renderContent = async function renderContent(uriPath) {
  ...
  const sections = {};
  for (const [_sectionToken, section] of entries) {
    for (let [fieldToken, field] of Object.entries(section.fields)) {
...
function _addPlaceholders(content, section, db) {
  ...
  if (content.length === 0) {
    const placeholders = Object.entries(section.fields).reduce((memo, [token, params]) => {
      ...
      if (!Record.SPECIAL_FIELDS.hasOwnProperty(params.key)) {
    ...
    for (let record of Object.values(content)) {
      for (let [id, value] of Object.entries(record)) {
