...
function ifHelper(input, options = {}) {
  const value = (typeof input === 'function') ? input() : input;
  return value ? options.fn(this) : options.inverse(this);
}

function unlessHelper(input, options = {}) {
  const value = (typeof input === 'function') ? input() : input;
  return ifHelper.call(this, !value, options);
}
...
function collectionHelper(input, options) {
  ...
  for (const item of items) {
    out += options.fn(this, { data: options.data, blockParams: [item] });
  }
...
function missingData(context, _options) {
  return `{{${context._context || 'general'}:${context.name}}}`;
}
...
class TemplateCompiler {
  ...
  parseFile(filePath) {
    ...
    const type = dirname(filePath).endsWith('collections') ? 'collection' : 'page';
      ...
      case 'BlockStatement': {
        switch (node.path.original) {
          ...
          case 'collection':
            ...
            if (node.program.blockParams && node.program.blockParams[0]) {
              aliases = Object.assign({}, aliases, {
                [node.program.blockParams[0]]: { name: node.params[0].original, type: 'collection' },
              });
            }
            if (node.program) _walk.call(this, tree, node.program, branchToken, aliases);
            if (node.inverse) _walk.call(this, tree, node.inverse, branchToken, aliases);
          ...
          case 'section':
            ensureBranch(tree, node);
            aliases = Object.assign({}, aliases, {
              '': { name: node.params[0].original, type: parseHash(node.hash).multiple === true ? 'collection' : 'setting' },
            });
...
function parseExpression(node) {
  ...
  switch (node.type) {
    ...
    case 'MustacheStatement':
      ...
      break;
    default: {
      throw new Error('Vapid Syntax Error');
    }
  }
...
function addToTree(tree, branchToken, node, aliases, options) {
  ...
  const name = (aliases[leaf.context] ? aliases[leaf.context].name : (leaf.context || branchToken)).toLowerCase();
  const type = (aliases[leaf.context] ? aliases[leaf.context].type : 'setting').toLowerCase();
  const sectionKey = `${type}:${name}`;
