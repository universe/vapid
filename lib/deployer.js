...
function _generateManifest(siteChecksums, presignedPosts) {
  return Object.entries(siteChecksums).reduce((memo, [path]) => {
...
async function _content(Section) {
  ...
  const order = Record.DEFAULT_ORDER.map((o) => { o.unshift('records'); return o; });
  const settings = await Section.scope('settings').findAll({ include: 'records', order });
  return settings.reduce((memo, section) => {
    /* eslint-disable-next-line no-param-reassign */
