...
const ICONS = {
  ...
  btn: <svg width="25px" height="25px">
    <path class="st0" d="M16.6,6c2.5,0,4.5,0,4.5,0c1.1,0,2,0.8,2,1.9V18c0,1.1-0.9,1.9-2,1.9H4C2.9,20,2,19.1,2,18V7.9C2,6.8,2.9,6,4,6h4.5 M9.1,7L4.2,7l0,0C3.6,7,3,7.6,3,8.2v9.7C3,18.5,3.6,19,4.2,19h16.7c0.6,0,1.1-0.5,1.1-1.1V8.2C22,7.5,21.5,7,20.9,7h-5"/>
...
interface HTMLDirectiveOptions {
  placeholder: string;
}

export default class HTMLDirective extends BaseDirective<string, HTMLDirectiveOptions> {
  default = '';
  ...
  input({ value = this.default, directive }: DirectiveProps<string, this>) {
    ...
    useEffect(() => {
      (async () => {
        ...
        const newQuill = new Quill(editor.current, options, { media: directive.meta.media.host });
    ...
    return <Fragment>
      <ul class="wysiwyg-blocks" ref={menu} tabIndex={-1} onfocusin={quill?.resetSelection}>
        <li><a href="#" class="wysiwyg-blocks__block wysiwyg-blocks__block--btn" onClick={(evt) =>insert(evt, quill?.buttonBlot) }>Button {ICONS.btn}</a></li>
        <li><a href="#" class="wysiwyg-blocks__block wysiwyg-blocks__block--hr" onClick={(evt) => insert(evt, quill?.hrBlot)}>Divider {ICONS.hr}</a></li>
        <li><a href="#" class="wysiwyg-blocks__block wysiwyg-blocks__block--img" onClick={(evt) => insert(evt, quill?.imgBlot)}>Image {ICONS.img}</a></li>
        <li><a href="#" class="wysiwyg-blocks__block wysiwyg-blocks__block--video" onClick={(evt) => insert(evt, quill?.videoBlot)}>Video {ICONS.vid}</a></li>
      </ul>
      <div class="wysiwyg" ref={editor} />
    </Fragment>;
  }
  ...
  preview(value: string = '') {
    let dirty: string = value.replace('<p><br></p>', '');
