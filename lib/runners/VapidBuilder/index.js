...
class VapidBuilder extends Vapid {
  ...
  async build(dest) {
    ...
    for (const html of htmlFile) {
      const parsed = path.parse(path.relative(this.paths.www, html));

      // If this is a partial, we need to check if it has a section object backing it.
      // If yes, render each record.
      if (Paths.isTemplatePartial(html)) {
        const name = parsed.name.slice(1);
        const collection = templates[`collection:${name}`];

        if (!collection) { continue; }

        const records = await collection.getRecords();
