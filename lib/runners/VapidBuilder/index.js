...
class VapidBuilder extends Vapid {
  ...
  async build(dest) {
    ...
    for (const asset of assets) {
      if (!Paths.isAssetPath(asset)) { continue; }
    ...
    for (const tmpl of templates) {
      ...
      // If this isn't a partial, it must be a page. Render it!
        ...
        await this.renderUrl(`/${parsed.name}`, path.join(dest, parsed.base));
        Logger.extra([`Created: /${parsed.base}`]);
      }
    }
    /* eslint-enable */
