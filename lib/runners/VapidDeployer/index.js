...
class VapidDeployer extends Vapid {
   ...
   */
  async ensureLogin() {
    const loggedIn = !!this.credentials;
  ...
  getChecksums() {
    ...
    glob.sync(path.resolve(this.tmpDir, '**/!(*.pack.+(s[ac]ss|js))'), { mark: true }).forEach((file) => {
      ...
      checksums[relativePath] = checksum(file);
    });
  ...
  async getContent() {
    ...
    const order = Template.DEFAULT_ORDER.map((o) => { o.unshift('records'); return o; });
    const contentSections = await Template.scope('content').findAll({ include: 'records', order });
    return contentSections.reduce((memo, template) => {
      /* eslint-disable-next-line no-param-reassign */
      memo[template.name] = template.records.map(record => record.get('content'));
      return memo;
    }, {});
  }
