...
router.use(async (ctx, next) => {
  if (ctx.isAuthenticated()) {
    ...
    const pages = await Record.findAll({
      ...
      order: [
        ['position', 'ASC'],
        [Sequelize.literal(`CASE WHEN template.name = '${Template.DEFAULT_PAGE}' THEN 1 ELSE 0 END`), 'DESC'],
      ],
    });

    // Just grab the info we need to render the pages side bar.
    ctx.state.pages = await Promise.all(pages.map(async p => ({
      id: p.id,
      permalink: (await p.permalink()).slice(1) || 'index',
      title: await p.title(),
      navigation: !!p.metadata.navigation,
    })));
