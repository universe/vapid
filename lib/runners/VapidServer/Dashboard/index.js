...
const { deploy } = require('@cannery/hoist');
...
router.get('deploy', '/deploy', async (ctx) => {
  const staticBuildPath = path.join(sitePaths.root, 'dist');
  console.log(`Building: ${sitePaths.root} to ${staticBuildPath}`);
  const builder = new VapidBuilder(sitePaths.root);
  await builder.build(staticBuildPath);
  const url = await deploy(staticBuildPath);
...
async function _content(ctx) {
  ...
  for (const file of ctx.request.files) {
    ...
    if (Utils.hasProperty(allowedFields, fieldName)) {
...
async function _saveFile(file) {
  ...
  const savePath = resolve(sitePaths.uploads, fileName);
