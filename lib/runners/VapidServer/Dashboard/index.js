...
router.use(async (ctx, next) => {
  if (ctx.isAuthenticated()) {
    ...
    ctx.state.pages = await Template.scope('pages').findAll({
      order: [
        [Sequelize.literal(`CASE WHEN name = '${Template.DEFAULT_PAGE}' THEN 1 ELSE 0 END`), 'DESC'],
        ['name', 'ASC'],
      ],
    });
...
router.get('records#new', '/:type/:name/records/new', findSection, async (ctx) => {
  const { template } = ctx.state;
  console.log('records#new', template);
  if (template.type === 'form') {
    const title = `${template.label} Form`;
...
router.get('/:type/:name/records/:id', findRecord, async (ctx) => {
  return _editRecordAction(ctx, ctx.state.record);
});

router.post('records#reorder', '/:type/:name/records/reorder', findSection, async (ctx) => {
  const { id, from, to } = ctx.request.body;
  const record = await Record.findByPk(id, { include: ['template'] });
...
router.post('records#create', '/:type/:name/records', findSection, async (ctx) => {
  ...
  try {
    ...
    record = await Record.build({ content, template_id: template.id });

    // TODO: Can't figure out how to get template to load for validation,
    // so I'm using the record.template = template hack
    ...
    ctx.flash('success', `Created ${template.labelSingular}`);
    return ctx.redirect(router.url('sections#index', template.typePlural, template.name));
  } catch (err) {
    console.error(err);
...
router.post('records#update', '/:type/:name/records/:id', findRecord, async (ctx) => {
  ...
  try {
    ...
    try { assert.deepStrictEqual(record.content, content); } catch (_err) {
      ...
      ctx.flash('success', `Updated ${template.labelSingular}`);
    }
...
router.post('/:type/:name/records/:id/delete', findRecord, async (ctx) => {
  ...
  ctx.flash('success', `Deleted ${ctx.state.template.labelSingular}`);
  ctx.redirect(router.url('sections#index', ctx.state.template.typePlural, ctx.state.template.name));
});
...
async function findSection(ctx, next) {
  ...
  if (template) {
    ...
    template.records = await template.getRecords();
...
async function _newRecordAction(ctx, errors = {}) {
  const title = ctx.state.template.repeating ? `New ${ctx.state.template.labelSingular}` : ctx.state.template.label;
  const record = Utils.isEmpty(ctx.request.body) ? { content: {} } : ctx.request.body;

  await ctx.render('records/new', title, {
    action: router.url('records#create', ctx.state.template.typePlural, ctx.state.template.name),
    form: 'content',
...
async function _editRecordAction(ctx, record = {}, errors = []) {
  const { template } = ctx.state;
  const title = template.labelSingular;
  await ctx.render('records/edit', title, {
    record,
    action: router.url('records#update', template.typePlural, template.name, record.id),
    deletePath: router.url('records#delete', template.typePlural, template.name, record.id),
    errors: _errors(errors),
    Form,
  });
}
